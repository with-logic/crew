...
| C-TAP-16d | §16.3 | `crew tap remove --uninstall <name>` uninstalls every attached entry at every scope, then removes the tap and its clone. With `--dry-run` nothing is removed: the installs, markers, `state.json`, and `config.yaml` are all unchanged. |
