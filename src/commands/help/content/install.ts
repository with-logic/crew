...
export const installHelp: CommandHelp = {
  ...
  notes: [
    ...
    "Browser links: a `/tree/<branch>/<folder>` link becomes `@<branch>//<folder>`, and a `/blob/.../SKILL.md` link points at the skill's folder. The first segment after `tree/` is taken as the branch, so for branch names with a `/` in them use the explicit `<repo-url>@<branch>//<folder>` form.",
  ],
