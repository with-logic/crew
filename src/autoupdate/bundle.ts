 ...
 * rather than "Homecrew". We can't change Bun's signature, but we *can*
 * give launchd a bundle to associate the agent with via the plist key
 * `AssociatedBundleIdentifiers`.
 *
 * We write a tiny no-executable bundle at `~/.crew/Homecrew.app/` whose only
 * purpose is to carry the display name we want Login Items to show.
 * macOS then attributes the agent to "Homecrew Skill Autoupdate" instead of the
 * binary's signer.
 *
...
export const BUNDLE_DISPLAY_NAME = "Homecrew Skill Autoupdate";

/** The absolute path to the bundle directory (inside `~/.crew/`). */
export function bundlePath(home: string): string {
  return join(paths(home).home, "Homecrew.app");
}

/**
 * Write the attribution bundle's `Info.plist`. Creates `Homecrew.app/Contents/`
...
<dict>
  ...
  <key>CFBundleExecutable</key><string>Homecrew</string>
