/**
 * `state.json` maintenance for a completed install (§11.1).
 *
 * `buildStateEntry` produces the entry for a freshly-installed skill;
 * `rebuildRequiredBy` rewrites the `required_by` edges across the
 * resolve set. Both are pure over the `StateFile` they receive; the
 * caller (`./index.ts`) holds the state lock and persists the result.
 */
